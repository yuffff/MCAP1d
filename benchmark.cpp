...
main(int argc, char *argv[])
{
  ...
  float  gosa;//measuring the errors?
   ...
   *    Start measuring
   */
  //gosa = jacobi(nn,&a,&b,&c,&p,&bnd,&wrk1,&wrk2);// the output in my programm is gosa 

  //printf("%.6f\n",gosa);//measuring the errors?
       ...
       Matrix* p,Matrix* bnd,Matrix* wrk1,Matrix* wrk2)
{
  ...
  for(n=0 ; n<nn ; n++){// nn stands for the number we are going to exec.
    gosa = 0.0;// error rate 
